...
-- start query 1 in stream 0 using template query14a.tpl
with cross_items as
 ...
 limit 100;
-- end query 1 in stream 0 using template query14a.tpl
-- start query 1 in stream 0 using template query14b.tpl
 with  cross_items as
 (select i_item_sk ss_item_sk
  from item,
     (select iss.i_brand_id brand_id, iss.i_class_id class_id, iss.i_category_id category_id
 ...
 limit 100;
-- end query 1 in stream 0 using template query14b.tpl
...
-- start query 1 in stream 0 using template query23a.tpl
 ...
 limit 100;
-- end query 1 in stream 0 using template query23a.tpl
-- start query 1 in stream 0 using template query23b.tpl
 with frequent_ss_items as
 (select substr(i_item_desc,1,30) itemdesc,i_item_sk item_sk,d_date solddate,count(*) cnt
  from store_sales, date_dim, item
  where ss_sold_date_sk = d_date_sk
 ...
 limit 100;
-- end query 1 in stream 0 using template query23b.tpl
-- start query 1 in stream 0 using template query24a.tpl
 with ssales as
...
-- end query 1 in stream 0 using template query24a.tpl
-- start query 1 in stream 0 using template query24b.tpl
 with ssales as
 (select c_last_name, c_first_name, s_store_name, ca_state, s_state, i_color,
         i_current_price, i_manager_id, i_units, i_size, sum(ss_net_paid) netpaid
 from store_sales, store_returns, store, item, customer, customer_address
 ...
 group by c_last_name, c_first_name, s_store_name
 having sum(netpaid) > (select 0.05*avg(netpaid) from ssales);
-- end query 1 in stream 0 using template query24b.tpl
...
-- start query 1 in stream 0 using template query39a.tpl
 with inv as
...
-- end query 1 in stream 0 using template query39a.tpl
-- start query 1 in stream 0 using template query39b.tpl
 with inv as
 (select w_warehouse_name,w_warehouse_sk,i_item_sk,d_moy
        ,stdev,mean, case mean when 0 then null else stdev/mean end cov
  from(select w_warehouse_name,w_warehouse_sk,i_item_sk,d_moy
 ...
 order by inv1.w_warehouse_sk,inv1.i_item_sk,inv1.d_moy,inv1.mean,inv1.cov
         ,inv2.d_moy,inv2.mean, inv2.cov;
-- end query 1 in stream 0 using template query39b.tpl
